...
class WiseChatChannelsTab extends WiseChatAbstractTab {
	...
	public function channelsChallback() {
		...
		if (count($summary) == 0) {
			...
			$html .= '<thead><tr><th>&nbsp;Name</th><th>Messages</th><th>Users</th><th>Last message</th><th width="200">Actions</th></tr></thead>';
		}
		...
		foreach ($summary as $key => $channel) {
			...
			$backupPrivateURL = $url.'&wc_action=backupPrivateChannel&channel='.urlencode($channel->channel);
			$html .= sprintf(
				'<tr id="%s" class="%s" style="display: none;">
					<td colspan="5">
						<span style="line-height: 2">Download:</span>
						<a class="button-secondary" href="%s" title="Public messages CSV">Public Messages</a> |
						<a class="button-secondary" href="%s" title="Private message CSV">Private Messages</a>
						<p class=\'description\'><strong>Notice:</strong> Backups CSV files are UTF-8 encoded and comma-separated</p>
					</td>
				</tr>',
				$channelBackupId, $classes, $backupURL, $backupPrivateURL
			);
		}
