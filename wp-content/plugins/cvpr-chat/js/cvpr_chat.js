/**
 * CVPR Chat core controller.
 *
 * @author Kainex <contact@kaine.pl>
 */
function CVPRChatController(options) {
	var progressBar = new CVPRChatProgressBar(options);
	var notifier = new CVPRChatNotifier(options);
	var messagesHistory = new CVPRChatMessagesHistory();
	var imageViewer = new CVPRChatImageViewer();
	var dateFormatter = new CVPRChatDateFormatter();
	var messageAttachments = new CVPRChatMessageAttachments(options, imageViewer, progressBar);
	var dateAndTimeRenderer = new CVPRChatDateAndTimeRenderer(options, dateFormatter);
	var messages = new CVPRChatMessages(options, messagesHistory, messageAttachments, dateAndTimeRenderer, notifier, progressBar);
	var settings = new CVPRChatSettings(options, messages);
	var maintenanceExecutor = new CVPRChatMaintenanceExecutor(options, messages, notifier);
    var emoticonsPanel = new CVPRChatEmoticonsPanel(options, messages);
...
/**
 * CVPRChatDateFormatter class. Formats dates given in UTC timezone.
 *
 * @author Kainex <contact@kaine.pl>
 */
function CVPRChatDateFormatter() {
...
/**
 * CVPRChatMessageAttachments class. Attachments management.
 *
 * @author Kainex <contact@kaine.pl>
 */
function CVPRChatMessageAttachments(options, imageViewer, progressBar) {
	var IMAGE_TYPES = ['jpg', 'jpeg', 'png', 'gif'];
	var prepareImageEndpoint = options.apiEndpointBase + '?action=Cvpr_chat_prepare_image_endpoint';
...
/**
 * CVPRChatNotifier - window title and sound notifiers.
 *
 * @author Kainex <contact@kaine.pl>
 */
function CVPRChatNotifier(options) {
...
/**
 * CVPRChatDateAndTimeRenderer - renders date and time next to each message according to the settings.
 *
 * @author Kainex <contact@kaine.pl>
 */
function CVPRChatDateAndTimeRenderer(options, dateFormatter) {
	...
	function formatFullDateAndTime(date, nowDate, element) {
		var timeFormatted = typeof options.messagesTimeFormat !== 'undefined' && options.messagesTimeFormat.length > 0
			? CvprChatUtilsMomentJS.moment(date).format(options.messagesTimeFormat)
		...
		if (dateFormatter.isSameDate(nowDate, date)) {
			...
			var dateFormatted = typeof options.messagesDateFormat !== 'undefined' && options.messagesDateFormat.length > 0
				? CvprChatUtilsMomentJS.moment(date).format(options.messagesDateFormat)
...
/**
 * CVPRChatProgressBar - controls the main progress bar.
 */
function CVPRChatProgressBar(options) {
	var container = jQuery('#' + options.chatId);
...
/**
 * CVPRChatEmoticonsPanel class is responsible for displaying emoticons layer and inserting selected emoticon
 * into message input field.
 *
 * @param {Object} options
 * @param {CVPRChatMessages} messages
 * @constructor
 */
function CVPRChatEmoticonsPanel(options, messages) {
    var EMOTICONS_1 = [
...
jQuery(window).load(function() {
	jQuery(".wcContainer[data-wc-config]").each(function(index) {
		...
		if (typeof config !== 'object') {
			jQuery(this).prepend('<strong style="color:#f00;">Error: invalid CVPR Chat configuration</strong>');
			return;
		}
		...
		if (typeof(window['CvprChatInstances']) == 'undefined') {
			window['CvprChatInstances'] = {};
		}
		
		if (typeof(window.CvprChatInstances[config.chatId]) == 'undefined') {
			window.CvprChatInstances[config.chatId] = new CVPRChatController(config);
		}
	});
});