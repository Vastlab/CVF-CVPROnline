...
/**
 * CVPR Chat links pre-filter.
 *
 * @author Kainex <contact@kaine.pl>
 */
class CVPRChatLinksPreFilter {
			...
			foreach ($matches[0] as $detectedURL) {
				...
				if ($detectAndDownloadImages && preg_match(self::URL_IMAGE_REGEXP, $detectedURL)) {
					...
					try {
						...
						$shortCode = CVPRChatShortcodeConstructor::getImageShortcode($result['id'], $result['image'], $result['image-th'], $detectedURL);
					} catch (Exception $ex) {
						$regularLink = true;
                        $actions = CVPRChatContainer::get('services/user/CVPRChatActions');
                        $authentication = CVPRChatContainer::get('services/user/CVPRChatAuthentication');
                    ...
                    $shortCode = CVPRChatShortcodeConstructor::getYouTubeShortcode($movieId, $detectedURL);
