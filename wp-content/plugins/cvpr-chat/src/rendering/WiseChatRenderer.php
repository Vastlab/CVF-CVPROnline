...
/**
 * CVPR Chat message rendering class.
 *
 * @author Kainex <contact@kaine.pl>
 */
class CVPRChatRenderer {
	...
	*/
	...
	public function __construct() {
		...
		$this->templater = new CVPRChatTemplater($this->options->getPluginBaseDir());
	}
	...
	public function getRenderedPasswordAuthorization($authorizationError = null) {
		$this->templater->setTemplateFile(CVPRChatThemes::getInstance()->getPasswordAuthorizationTemplate());
	...
	*/
	public function getRenderedAccessDenied($errorMessage, $cssClass) {
		$this->templater->setTemplateFile(CVPRChatThemes::getInstance()->getAccessDeniedTemplate());
	...
	public function getRenderedUserNameForm($errorMessage = null) {
		$this->templater->setTemplateFile(CVPRChatThemes::getInstance()->getUserNameFormTemplate());
		$data = array(
			'themeStyles' => $this->options->getBaseDir().CVPRChatThemes::getInstance()->getCss(),
			'windowTitle' => $this->options->getEncodedOption('window_title', 'CVPR Chat'),
			'errorMessage' => $errorMessage,
	...
	*/
	public function getRenderedMessage($message) {
		$this->templater->setTemplateFile(CVPRChatThemes::getInstance()->getMessageTemplate());
	...
	*/
	private function getRenderedMessageContent($message) {
        ...
        /** @var CVPRChatLinksPostFilter $linksFilter */
        $linksFilter = CVPRChatContainer::get('rendering/filters/post/CVPRChatLinksPostFilter');
		$formattedMessage = $linksFilter->filter(
            $formattedMessage,
            $this->options->isOptionEnabled('allow_post_links', true)
        );

        /** @var CVPRChatAttachmentsPostFilter $attachmentsFilter */
        $attachmentsFilter = CVPRChatContainer::get('rendering/filters/post/CVPRChatAttachmentsPostFilter');
        ...
        /** @var CVPRChatImagesPostFilter $imagesFilter */
        $imagesFilter = CVPRChatContainer::get('rendering/filters/post/CVPRChatImagesPostFilter');
        ...
        /** @var CVPRChatYouTubePostFilter $youTubeFilter */
        $youTubeFilter = CVPRChatContainer::get('rendering/filters/post/CVPRChatYouTubePostFilter');
		...
		if ($this->options->isOptionEnabled('enable_twitter_hashtags', true)) {
            /** @var CVPRChatHashtagsPostFilter $hashTagsFilter */
            $hashTagsFilter = CVPRChatContainer::get('rendering/filters/post/CVPRChatHashtagsPostFilter');
			$formattedMessage = $hashTagsFilter->filter($formattedMessage);
		}

		$emoticonsSet = $this->options->getIntegerOption('emoticons_enabled', 1);
		if ($emoticonsSet > 0) {
            /** @var CVPRChatEmoticonsFilter $emoticonsFilter */
            $emoticonsFilter = CVPRChatContainer::get('rendering/filters/post/CVPRChatEmoticonsFilter');
