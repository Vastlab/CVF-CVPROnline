...
/**
 * CVPRChat and WordPress users DAO.
 *
 * @author Kainex <contact@kaine.pl>
 */
class CVPRChatUsersDAO {
	/**
	* @var CVPRChatOptions
	*/
	private $options;
	...
	public function get($id) {
		...
		$table = CVPRChatInstaller::getUsersTable();
	...
	/**
	 * Converts raw object into CVPRChatUser object.
	 *
	...
	*/
	public function hasCurrentWpUserRight($rightName) {
		...
		if ($wpUser !== null) {
			...
			if ((is_array($wpUser->roles) && count(array_intersect($targetRoles, $wpUser->roles)) > 0) || current_user_can("Cvpr_chat_{$rightName}")) {
				return true;
			}
		}
