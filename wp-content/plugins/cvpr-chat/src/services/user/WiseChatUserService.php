...
/**
 * CVPRChat user services.
 *
 * @author Kainex <contact@kaine.pl>
 */
class CVPRChatUserService {
	const USERS_ACTIVITY_TIME_FRAME = 80;
	const USERS_PRESENCE_TIME_FRAME = 86400;
	const USERS_LIST_SESSION_KEY = 'Cvpr_chat_current_users_list_channel_';
	...
	*/
	public function initMaintenance() {
		$userSettingsDAO = CVPRChatContainer::get('dao/user/CVPRChatUserSettingsDAO');
	...
	*/
	private function refreshNewUserName($user) {
        ...
        $this->messagesDAO->updateUserNameByCriteria(
			$user->getName(), CVPRChatMessagesCriteria::build()->setUserId($user->getId())
		);

        /** @var CVPRChatRenderer $renderer */
        $renderer = CVPRChatContainer::get('rendering/CVPRChatRenderer');
