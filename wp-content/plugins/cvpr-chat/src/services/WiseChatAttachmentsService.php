...
/**
 * CVPR Chat attachments services class.
 *
 * @author Kainex <contact@kaine.pl>
 */
class CVPRChatAttachmentsService {
	const UPLOAD_FILE_NAME = '_Cvpr_chat_upload_attachment';
	...
	*/
	public function markAttachmentsWithDetails($attachmentIds, $channel, $messageId) {
		foreach ($attachmentIds as $attachmentId) {
			add_post_meta($attachmentId, 'Cvpr_chat_message_id', $messageId);
			add_post_meta($attachmentId, 'Cvpr_chat_channel', $channel);
			add_post_meta($attachmentId, 'Cvpr_chat', '1');
		}
	}
	...
	*/
	private function deleteAttachemnts($attachments) {
		foreach ($attachments as $attachment) {
			$meta = get_post_meta(intval($attachment->ID), 'Cvpr_chat');
	...
	*/
	private function logError($message) {
		trigger_error('WordPress CVPR Chat plugin error: '.$message, E_USER_NOTICE);
