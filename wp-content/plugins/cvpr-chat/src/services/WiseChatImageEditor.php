...
/**
 * CVPR Chat images editor class.
 *
 * @author Kainex <contact@kaine.pl>
 */
class CVPRChatImageEditor {
	/**
	 * GD Resource.
	 *
	 * @var resource
	...
	*/
	public function load($imagePath) {
		if (!file_exists($imagePath)) {
			throw new Exception('CVPRChatImageEditor: File does not exist');
		}
		
		$imageData = getimagesize($imagePath);
		if ($imageData === false) {
			throw new Exception('CVPRChatImageEditor: The file is not an image');
		}
		...
		switch ($imageData[2]) {
			...
			case IMAGETYPE_WBMP:
				...
				break;
			default:
				throw new Exception('CVPRChatImageEditor: Image format is not supported');
		}
	...
	*/
	public function resize($maxWidth, $maxHeight, $constraint = true) {
		if ($this->image === null) {
			throw new Exception('CVPRChatImageEditor: Image was not loaded');
		}
