...
/**
 * CVPR Chat admin channels settings tab class.
 *
 * @author Kainex <contact@kaine.pl>
 */
class CVPRChatChannelsTab extends CVPRChatAbstractTab {
    ...
    public function deleteChannelAction() {
        ...
        $this->addMessage(
            ...
            Note: You have to remove [Cvpr-chat] shortcode as well. If you don\'t remove it the channel will be created again when someone enters the page that contains [Cvpr-chat] shortcode.');
    }
	
	public function backupChannelAction() {
		$channel = $_GET['channel'];
		$channelStripped = preg_replace("/[^[:alnum:][:space:]]/ui", '', $channel);
		$filename = "CVPRChatChannelBackup-{$channelStripped}.csv";
	...
	public function channelsChallback() {
		$url = admin_url("options-general.php?page=".CVPRChatSettings::MENU_SLUG);
		...
		foreach ($summary as $key => $channel) {
			...
			$setPasswordURL = admin_url("options-general.php?page=".CVPRChatSettings::MENU_SLUG."&wc_action=setChannelPassword&channel=".urlencode($channel->channel));
			$deletePasswordURL = admin_url("options-general.php?page=".CVPRChatSettings::MENU_SLUG."&wc_action=deleteChannelPassword&channel=".urlencode($channel->channel));
	...
	public function adminActionsCallback() {
		$url = admin_url("options-general.php?page=".CVPRChatSettings::MENU_SLUG."&wc_action=clearAllChannels");
