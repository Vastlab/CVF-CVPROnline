...
class TitanFrameworkCSS {
	...
	public function enqueueCSS() {
		...
		if ( $this->frameworkInstance->settings['css'] == 'generate' ) {
			...
			$get_timestamp = get_option('ect_timestamp');
			$set_timestamp = isset($get_timestamp)?$get_timestamp:time();
			// var_dump($generatedCSS);

			if ( file_exists( $generatedCss ) && empty( $css ) ) {
				wp_enqueue_style( 'tf-compiled-options-' . $this->frameworkInstance->optionNamespace, $this->getCSSFileURL(), __FILE__,$set_timestamp,'all');
			}
		}
	}
