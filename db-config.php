...
//TB mod if write host not defined then make it host
// makes it easuer to ease consistently in  a mixed config..  replicates get both defined, master gets only host then
 

if(! defined('DB_HOST')) define('DB_HOST',WP_HOST);
if(! defined('DB_WRITE_HOST')) define('DB_WRITE_HOST',DB_HOST);

/**
 * This adds thelocal again, only this time it is configured as a slave.
...
$wpdb->add_database(array(
	'host'     => DB_WRITE_HOST,     // If port is other than 3306, use host:port.
	'user'     => DB_USER,
	'password' => DB_PASSWORD,
	'name'     => DB_NAME,
	'write'    => 1,
...
/*
 $wpdb->add_database(array(
 	...
 	'write'    => 1,
 	'read'     => 1,
 	'dataset'  => 'wp_chats',  // chats get everything from master so there is less delay.. its ajax anyhow 
 	'timeout'  => 0.2,
 ));
...
function resolve_with_dataset ($query, $wpdb) {
  // 
    if ( preg_match("/wp_wise_chat/", $wpdb->table) ) {
            return 'wp_chats';
    } else {
        return null;
   }
